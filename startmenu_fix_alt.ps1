#*******************************************************************************************#
# IMPORTANT: This version is for the case where Windows designated shortcuts from CURRENT   #
...
Write-Host "Start Menu Fix (Alt Ver.) Starting..."
Write-Host "==============================`n"
...
function cleaning-folder {
    ...
    $bad_files = dir -Filter "* (1).lnk*"
    if ($bad_files.Length -gt 0) {
        Write-Host $("    "*$level + "Duplicate Found! Cleaning Folder...")
        # remove files added to bad file list
        foreach ($bad_file in $bad_files){
            move-to-trash $bad_file.toString()
        }
        Write-Color-Host red $("    "*$level + ("Removed {0} Duplicates!" -f $bad_files.Length))
    } else {
